...
public class Player {
	...
	public Player(String firstName, String lastName, String nickName, LocalDate dob, Position position) {
		this(firstName, lastName, nickName, dob, Position.NONE, DEFAULT_ATTR_VALUES, DEFAULT_ATTR_VALUES,
				DEFAULT_ATTR_VALUES, DEFAULT_ATTR_VALUES, DEFAULT_ATTR_VALUES, DEFAULT_ATTR_VALUES);
	}

	public Player(String firstName, String lastName, String nickName, LocalDate dob, Position position, int speed,
			int shooting, int tackling, int passing, int reactions, int blocking) {
		m_id = ID_COUNTER;
		ID_COUNTER++;
	...
	private boolean calculateOverall() {
		...
		if (getPositionEnum() == Position.ATT) {
			...
			weightSum = ATT_SPEED_WEIGHT + ATT_SHOOT_WEIGHT + ATT_TACKL_WEIGHT + ATT_PASS_WEIGHT + ATT_REACT_WEIGHT
					+ ATT_BLOCK_WEIGHT;
		}
			...
			weightSum = MID_SPEED_WEIGHT + MID_SHOOT_WEIGHT + MID_TACKL_WEIGHT + MID_PASS_WEIGHT + MID_REACT_WEIGHT
					+ MID_BLOCK_WEIGHT;
		}
			...
			weightSum = DEF_SPEED_WEIGHT + DEF_SHOOT_WEIGHT + DEF_TACKL_WEIGHT + DEF_PASS_WEIGHT + DEF_REACT_WEIGHT
					+ DEF_BLOCK_WEIGHT;
		}
			...
			weightSum = GK_SPEED_WEIGHT + GK_SHOOT_WEIGHT + GK_TACKL_WEIGHT + GK_PASS_WEIGHT + GK_REACT_WEIGHT
					+ GK_BLOCK_WEIGHT;
		}
			...
			weightSum = NONE_SPEED_WEIGHT + NONE_SHOOT_WEIGHT + NONE_TACKL_WEIGHT + NONE_PASS_WEIGHT + NONE_REACT_WEIGHT
					+ NONE_BLOCK_WEIGHT;
		}

		m_overall = (m_speedWeighted + m_shootWeighted + m_tacklWeighted + m_passWeighted + m_reactWeighted
				+ m_blockWeighted) / weightSum;
	...
	public boolean isValid() {
		if (m_position != Position.ATT && m_position != Position.MID && m_position != Position.DEF
				&& m_position != Position.GK && m_position != Position.NONE) {
			System.err.println("model.Player Error: position is not valid");
			return false;
		}

		if (m_seasonGoals < 0) {
			System.err.println("model.Player Error: season goals cannot be negative");
			return false;
		}

		if (m_seasonAssists < 0) {
			System.err.println("model.Player Error: season assists cannot be negative");
			return false;
		}

		if (m_seasonSaves < 0) {
			System.err.println("model.Player Error: season saves cannot be negative");
			return false;
		}

		if (m_seasonTackles < 0) {
			System.err.println("model.Player Error: season tackles cannot be negative");
			return false;
		}

		if (m_overall < 1 || m_overall > 99) {
			System.err.println("model.Player Error: overall must be between 1 and 99 inclusive");
			return false;
		}

		if (m_speed < 1 || m_speed > 99) {
			System.err.println("model.Player Error: speed must be between 1 and 99 inclusive");
			return false;
		}

		if (m_shooting < 1 || m_shooting > 99) {
			System.err.println("model.Player Error: shooting must be between 1 and 99 inclusive");
			return false;
		}

		if (m_tackling < 1 || m_tackling > 99) {
			System.err.println("model.Player Error: tackling must be between 1 and 99 inclusive");
			return false;
		}

		if (m_passing < 1 || m_passing > 99) {
			System.err.println("model.Player Error: passing must be between 1 and 99 inclusive");
			return false;
		}

		if (m_reactions < 1 || m_reactions > 99) {
			System.err.println("model.Player Error: reaction must be between 1 and 99 inclusive");
			return false;
		}

		if (m_blocking < 1 || m_blocking > 99) {
			System.err.println("model.Player Error: blocking must be between 1 and 99 inclusive");
			return false;
		}

		if (m_team != null && m_isFreeAgent) {
			System.err.println("model.Player Error: player cannot have a team and be a free agent");
			return false;
		}

		if (m_team == null && !m_isFreeAgent) {
			System.err.println("model.Player Error: team cannot be null and player cannot be a free agent");
			return false;
		}
