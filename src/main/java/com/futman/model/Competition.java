...
public abstract class Competition {
	...
	private int m_maxMatchesSeason;
	...
	public boolean addTeam(Team team) {
		if (findTeam(team) < 0 && m_teamCount < MAX_SIZE) {
			...
			System.err.println("model.Competition Error: couldn't add team");
			return false;
		}
	...
	public boolean startNewSeason() {
		...
		if (!(m_seasons.get(m_seasonCount - 1).startSeason())) {
			System.err.println("model.Competition Error: unable to start season");
			return false;
		}
	...
	public boolean startNextSeason() {
		if (!endCurrentSeason()) {
			System.err.println("model.Competition Error: unable to end current season");
			return false;
		}
		...
		if (!startNewSeason()) {
			System.err.println("model.Competition Error: unable to start new season");
			return false;
		}
	...
	public boolean isValid() {
		if (m_name.length() <= 0) {
			System.err.println("model.Competition Error: name cannot be empty");
			return false;
		}

		if (m_country == null) {
			System.err.println("model.Competition Error: country cannot be null");
			return false;
		}

		if (m_teams == null) {
			System.err.println("model.Competition Error: teams cannot be null");
			return false;
		}

		if (m_teamCount < 0) {
			System.err.println("model.Competition Error: team count cannot be negative");
			return false;
		}

		for (int x = 0; x < m_teamCount; x++) {
			if (m_teams[x] == null) {
				System.err.println("model.Competition Error: team at index " + x + " cannot be null");
				return false;
			}
		}
		...
		for (int x = m_teamCount; x < MAX_SIZE; x++) {
			if (m_teams[x] != null) {
				System.err.println("model.Competition Error: team at index " + x + " must be null, after current team size");
				return false;
			}
		}

		if (m_maxMatchesSeason < 0) {
			System.err.println("model.Competition Error: max matches cannot be negative");
			return false;
		}

		if (m_seasons == null) {
			System.err.println("model.Competition Error: seasons cannot be null");
			return false;
		}

		if (m_seasonCount < 0) {
			System.err.println("model.Competition Error: season count cannot be negative");
			return false;
		}
